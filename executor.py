...
class ExecutorAgent:
    ...
    def _detect_unreachable_code_after_return(
        ...
        for i, line in enumerate(non_empty_adds):
            stripped = line.strip()
            is_terminal = (stripped.startswith("return ") or stripped == "return" or stripped.startswith("raise ") or stripped.startswith("break") or stripped.startswith("continue"))
            if not is_terminal:
                continue
            ...
            for following in non_empty_adds[i + 1:]:
                ...
                if self._count_indent(following) >= terminal_indent:
                    return True, {"terminal_line": stripped, "dead_line": following_stripped, "reason": "additions contain executable code after a terminal statement"}
                break
