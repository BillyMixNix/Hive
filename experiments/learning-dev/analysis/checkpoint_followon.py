"""Descriptive follow-on on untouched tasks after the documented ID-copy defect.

No replay or rescued confirmatory claim. All earlier spend and failures remain.
"""
import copy
import hashlib
import json
...
INITIAL_PRIOR_NUSD = 2_639_946_400
BANK_SHA256 = "53e959ee24c1c0fca0be1f899ce711f2636f01c3925ed5e9df3cd4d5cb6936a4"


def sources():
    names = ["analysis/checkpoint_followon.py", "analysis/indexed_checkpoint.py", "analysis/lesson_bank_v5.py", "analysis/lesson_checkpoint.py", "analysis/checkpoint_cases.py",
...
def read_plan():
    plan = strict_json((ROOT/"checkpoint-followon-plan.json").read_bytes())
    phase = plan["phase"]
    if (type(phase) is not int or not 2 <= phase <= 4
            or plan["launch_message"] != f"Run authorized Hive checkpoint followon phase {phase}"
    ...
    if (study["schema"] != "hive.checkpoint-followon.v1" or study["model"] != MODEL
            or study["calls_per_recipient"] != 36 or len(study["cases"]) != 15
    ...
    if len(set(keys)) != 15 or sum(c["split"] == "confirmation" for c in study["cases"]) != 12:
    ...
    if phase > 2:
        ...
        if (prior["study_state_sha256"] != plan["prior_state_sha256"]
                ...
                or [p["phase"] for p in previous["phases"]] != list(range(2, phase))
          ...
          or spent != INITIAL_PRIOR_NUSD or prior["episode_id"] != "d23ea9f39bede9745980bbd6ac55bb83de04d6b79411bbf9ed1eb80da90d465a"):
        raise ValueError("follow-on must carry completed V5 phase-one account exactly")
    return plan, study, prior, previous


def describe(state, study):
    target = {(c["id"], arm) for c in study["cases"] for arm in ARMS}
    keys = [(r["case_id"], r["arm"]) for r in state["trials"]]
    if len(set(keys)) != len(keys) or set(keys) != target:
        return {"verdict": "INCOMPLETE"}
    if not all(r["integrity_valid"] for r in state["trials"]):
        return {"verdict": "INVALID"}
    by = {(r["case_id"], r["arm"]): r for r in state["trials"]}
    transfer = [c["id"] for c in study["cases"] if c["split"] == "confirmation"]
    retention = [c["id"] for c in study["cases"] if c["split"] == "retention"]
    def score(r):
        return r["usage"]["calls"] if r["passed"] else 36
    comparisons = {}
    for arm in ("baseline", "neutral"):
        ...
        lesson = sum(score(by[c, "lesson"]) for c in transfer)
        comparisons[arm] = {"control_penalized_calls": control, "lesson_penalized_calls": lesson,
            "relative_reduction": 1-lesson/control, "paired_differences": differences,
    ...
    return {"verdict": "DESCRIPTIVE_ONLY", "confirmatory_gain_claim_allowed": False,
        ...
        "interpretation": "The original V5 confirmation stopped after an interface defect. These untouched tasks use a changed handle schema and are reported separately. No alpha is reallocated and no confirmatory gain claim is made."}
...
def main():
    ...
    report = {"episode_id": digest(manifest), "scope": "descriptive_followon_after_short_memory_handle_repair",
    ...
    state = copy.deepcopy(previous) if previous else {"schema": "hive.checkpoint-followon.state.v1",
    ...
    try:
        ...
        for case, arm in schedule:
            ...
            adapter = IndexedCheckpointHive(MODEL, key, max_requests=36, spending=guard,
            ...
            try:
                ...
                print("HIVE_CHECKPOINT_PROGRESS " + json.dumps({"case": case["id"], "arm": arm,
    ...
    print("HIVE_CHECKPOINT_REPORT " + json.dumps({k: v for k, v in report.items() if k != "transport_usage"}), flush=True)
